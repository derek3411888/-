...
CallAiChatByPowerShell(apiUrl, apiKey, model, userPrompt) {
    ...
    if (provider = "gemini") {
        ...
        script .= "  foreach ($p in $priority) { if ($autoCandidates -contains $p) { $candidates += $p } }`n"
        script .= "  foreach ($x in $autoCandidates) { if ($candidates -notcontains $x) { $candidates += $x } }`n"
        script .= "  if (-not [string]::IsNullOrWhiteSpace($model) -and $candidates -notcontains $model) { $candidates += $model }`n"
        script .= "  if (-not [string]::IsNullOrWhiteSpace($geminiModelsCsv)) { foreach ($x in $geminiModelsCsv.Split(',')) { if (-not [string]::IsNullOrWhiteSpace($x) -and $candidates -notcontains $x) { $candidates += $x } } }`n"
