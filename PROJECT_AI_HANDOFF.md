...
- 5.05／5.16／1.0.68：原程式已抓到 3.7 的 09/30 04:00～11:00（UTC+8）官方公告，但只挑當日事件傳給網站，導致提前公告看不到。新增 `upcomingNotice` 獨立預覽，`GameMaintenancePolicy.ahk` 的當日等待判斷保持不變；不能將未來預覽當成目前 eventId 或提早阻擋鋤地。
- 5.04 的首次正式 HTTP 驗收發現中央網站漏列 `/game-maintenance-view.js` 靜態檔路由，已回復舊 API 並以 5.05 完整重發，不覆寫舊版 artifact。新增瀏覽器 import／HTTP 路由對應測試，以及部署後實際讀取 JS MIME/body 的驗收；單靠 `/health/ready` 不代表網頁可載入。
...
- 最後整合修正以 [審查／回歸紀錄](docs/game-maintenance-final-review.md) 為準：等待／慢 OCR 期間已 ACK 的意圖不能被舊狀態覆蓋；維護期 canonical 遊戲身分與 F11 送鍵前策略都重驗。F11 接續必須是原 OKWW 建立身分加目標主畫面，不重開 manager；舊 journal 無身分證據時不重送。
