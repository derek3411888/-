...
- 啟動 `自動開啟OKWW.ahk` 後，最多等待 45 秒，只接受由 `pythonw.exe` 承載、標題完整符合 `OK-WW v版本 Global ... - OK-WW` 的最終主視窗。
- 同一個 HWND 必須連續確認兩次才算穩定；確認後只送出一次 `F11`。優先切到前景以 `SendEvent` 發送，無法切前景時才以 `ControlSend` 定向發送。
- F11 發送結果會記入全自動日誌；若找不到最終主視窗或送鍵失敗，`OKWW流程` 會記為失敗，不再誤報成功。
