...
總覽新增「遊戲版本維護」狀態卡，設定頁新增維護開關、重新查詢、當次延後與略過時間。Steam／官方版來源由裝置按實際啟動入口辨識；舊版、離線或未回報 capability v1 的裝置不開放新操作。

使用既有 `gameMaintenanceJson` 心跳欄位與設定 revision，不新增 Firestore 輪詢；倒數在瀏覽器本地計算。網頁明確區分已送出、已 ACK／套用和拒絕原因，未知更新百分比不顯示假進度。手動延後最多 48 小時，綁定目前事件；略過不等於略過版本、安全或公告來源檢查。

`game-maintenance-view.js` 與自架網站共用相同內容，打包時同步並驗證 SHA；網站與新版 Payload 都發布後才有完整功能。真實 Steam／Kuro 更新的實機驗收仍是獨立交付門檻，不能由網站設定繞過。
...
頁首的「通知目前 Codex 任務」不會另開 ChatGPT。可以選擇「找出問題並修正／只分析原因／檢查目前狀態」三種預設訊息，也可以自行輸入最多 1,000 字元；Docker／資料庫所在的 Windows 主機常駐橋接程式收到後，會將訊息保存到指定既有任務的 Codex 持久佇列，由已開啟的 Codex 桌面版接手。橋接不會用另一個短命 App Server 恢復或啟動模型，也不會嘗試啟動桌面套件未提供的 standalone daemon。

網頁會分別顯示「網站已送出／家中主機收到／訊息驗證／送往 Codex／Codex Turn 已建立」，並列出 nonce、各階段時間、嘗試次數、下次重試、訊息 SHA-256 指紋、Turn ID 與錯誤原因。入列 ACK 只會顯示「已排隊，尚未開始」；只有相同訊息確實出現在指定聊天室的 Turn 紀錄，才顯示正在處理。最終回覆也必須屬於同一個請求與 Turn。重試先查既有佇列及 Turn，不重複新增；兩次成功入列至少間隔 5 分鐘。明確的傳送前錯誤最多自動重試 3 次、至少間隔 30 秒；入列結果不明時停止重送以避免重複執行。請保持目標聊天室在 Codex 桌面版開啟；關閉、未載入或中斷的聊天室不能僅凭佇列 ACK 宣告已啟動。舊版網頁的 `FIX_SCRIPT` 動作仍可使用。安裝方式見 `self-hosted-server/windows/Install-CodexSupportBridge.ps1`。
...
- 「診斷記錄」：目前效能摘要、最近 60 分鐘圖表、最近流程事件與命令歷史；本機單檔錄影狀態／路徑預設收起。
- 「設定」：可安全遠端同步的執行端設定。

三頁共用固定在上方的裝置選擇列。桌面版的總覽與診斷使用雙欄，窄螢幕與手機會自動堆疊；事件與命令表格在手機上會改成卡片，不需要橫向捲動。

介面採用一致的 Apple 風格視覺系統：半透明卡片、分段式頁籤、清楚的狀態色與輕量進場動畫。鍵盤可在頁籤上使用左右方向鍵、Home 與 End 切換；系統開啟「減少動態效果」時會自動停用非必要動畫。版面以 1080p 桌面與 390px 手機寬度作為實際驗證基準，所有主要操作維持可見且不產生水平捲動。
...
效能採集仍在本機每 2 秒執行，但公司頁只接收目前摘要與最近 60 個每分鐘彙整點。精簡 JSON 設有 32 KiB 硬上限，直接併入原有 90 秒 client 心跳，因此不新增 Firestore 文件、固定寫入、listener 或 poll；公司頁沿用既有 client listener 顯示圖表。自架 PostgreSQL 仍保留最長 14 天的完整分鐘資料，公司頁只顯示最近一小時。
...
網路短影片與中央正式影片上傳都已停用。FFmpeg 從開始便把一個 MKV 直接寫到設定的本機或 UNC 目的地；網站錄影狀態卡只顯示目前單檔、指定資料夾與封口結果，並提供複製路徑按鈕。
